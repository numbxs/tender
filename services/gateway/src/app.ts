 ...
 * making agent-native.
 *
 * Runs on two runtimes from the same app: Cloudflare Workers (deployed, via
 * src/index.ts) and Node (local dev, via src/local.ts). Workers has no
 * process.env -- request-time config lives only in Hono's `c.env`, populated
 * from wrangler.jsonc's `vars` -- so config is resolved per-request from
 * `c.env`, falling back to `process.env` only where that global actually
 * exists (Node). Nothing reads env at module scope.
 */
...
type Bindings = {
  X402_NETWORK?: string;
  X402_ASSET?: string;
  X402_BID_PRICE_USDC?: string;
  X402_PAY_TO_ADDRESS?: string;
};
type Vars = { payment: PaymentProof };
type Env = { Bindings: Bindings; Variables: Vars };

function envVar(c: Context<Env>, key: keyof Bindings): string | undefined {
  const fromBindings = c.env?.[key];
  if (fromBindings !== undefined) return fromBindings;
  // @hono/node-server's local dev doesn't populate c.env from process.env, so
  // this fallback is what makes X402_PAY_TO_ADDRESS etc. work in `pnpm dev`.
  if (typeof process !== "undefined") return process.env[key];
  return undefined;
}

function resolveConfig(c: Context<Env>): X402Config {
  const config: X402Config = {
    network: envVar(c, "X402_NETWORK") ?? HEDERA_TESTNET,
    payTo: envVar(c, "X402_PAY_TO_ADDRESS") ?? "0x0000000000000000000000000000000000000000",
    // Hedera assets are token ids, not symbols. Testnet USDC is 0.0.429274 (6dp).
    asset: envVar(c, "X402_ASSET") ?? HEDERA_TESTNET_USDC_ASSET,
    priceUsdc: envVar(c, "X402_BID_PRICE_USDC") ?? "0.01",
  };
...
app.get("/bids/quote", (c) => {
  ...
  return c.json({ priceUsdc: config.priceUsdc, asset: config.asset, network: config.network });
});
...
app.post("/bids", paymentRequired(resolveConfig), async (c) => {
